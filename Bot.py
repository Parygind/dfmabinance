...
def updateData():
    ...
    for pr in tickers:
        if tickers[pr]['symbol'][-4:] == 'USDT' and float(tickers[pr]['quoteVolume']) >= 2000000 and float(
                tickers[pr]['quoteVolume']) <= 5500000000000 and float(tickers[pr]['bidVolume']) > 0 \
                and tickers[pr]['symbol'] != 'SUSD/USDT' and tickers[pr]['symbol'] != 'LINK/BTC' and tickers[pr][
            'symbol'] != 'DCR/BTC' and tickers[pr]['symbol'] != 'BTC/USDT' \
...
def alarm2(context):
    ...
    mesShort = ''
    job = context.job
    global dict_prev, dict_curr, symb_list, c, tk, sl, dict_order, dict_pass, dict_prec, dict_start_price, dict_max_price, dict_min_price, dict_prev_vol, dict_prev_min, dict_last, trade_on
                    ...
                    if symb not in dict_order and symb not in dict_pass and symb != 'BTCBUSD':
                        ...
                        for i, e in reversed(list(enumerate(dict_list[symb]))):
                            if (t - e[0]) / 1000 <= 30:
                                ...
                                if step == 2:
                                    if prevVol >= dict_curr[symb] * (0.19 * (30/60)):
                                        try:
                                ...
                                if step == 3:
                                    if prevVol >= dict_curr[symb] * (0.019 * (45/60)):
                                        try:
                                            inf = get_klines1(symb, '1m', None, 5)
                                        except:
                                            break
                                ...
                                if prevVol >= dict_curr[symb] * 0.019:
                                    try:
                                        inf = get_klines1(symb, '1m', None, 5)
                                    except:
                                        break
