...
def updateData(context):
    ...
    for pr in tickers:
        if tickers[pr]['symbol'][-4:] == 'USDT' and float(tickers[pr]['quoteVolume']) >= 300000 and float(tickers[pr]['quoteVolume']) <= 10000000 and float(tickers[pr]['bidVolume']) > 0 and float(tickers[pr]['high']) < 20 \
...
def alarm2(context):
    ...
    for i in range(0, int(len(symb_list))):
        ...
        if vol >= dict_curr[symb_list[i]] * 0.02 and float(inf[0][2]) / float(inf[0][1]) < 1.06 and float(inf[0][4]) / float(inf[0][1]) > 0.98 and len(dict_order) < 7:
            passPair = False
